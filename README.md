...
W pliku store.c (funkcja store_init) alokowana będzie pamięć na listę sąsiedztwa czyli n struktur, gdzie n oznacza liczbę wierzchołków grafu przy czym każda kolejna struktura rozpoczynając od 0 będzie zawierała długość najkrótszej drogi z wierzchołka źródłowego do wierzchołka o tym indeksie i indeks wierzchołka, z którego wcześniej poprowadzono krawędź do aktualnego wierzchołka oraz wskaźnik na drugą strukturę, (którą uzupełniać będzie funkcja store_init_edge z pliku store.c) która przechowywać będzie indeks wierzchołka, na który możemy z danego wierzchołka przejść oraz wagę krawędzi przejścia na ten wierzchołek oraz wskaźnik na następną taką strukturę (jeżeli istnieje, bo dany wierzchołek może już nie mieć więcej krawędzi). Dane do drugiej struktury będą zapisywane z pliku lub z wygenerowanych wartości dla grafu. 
Przykładowo: dla wybranego wierzchołka źródłowego - 0, po zaalokowaniu pamięci na n struktur, gdzie zmienna lista jest typu wskaźnik na strukturę,  lista[0] - struktura odnosząca się do wierzchołka o indeksie 0 przechowuje długość najkrótszej drogi z wierzchołka źródłowego (0) do wierzchołka o tym indeksie (0) i indeks wierzchołka, z którego wcześniej poprowadzono krawędź do aktualnego wierzchołka (0, ale dla np. wierzchołka o indeksie 3 z obrazka poniżej po wykonaniu algorytmu Dijkstry będzie to 1, bo najkrótsza droga od wierzchołka 0 do wierzchołka 3 to 5 i jest to droga 0-1-3) oraz wskaźnik na głowę drugiej struktury, która przechowuje indeks wierzchołka, na który z danego wierzchołka możemy przejść (dla poniższego obrazka z wierzchołka 0 możemy przejść tylko na 1 i 2) i wagę krawędzi przejścia na ten wierzchołek (dla poniższego obrazka z 0 na 1 waga: 3, z 0 na 2 waga: 5) oraz wskaźnik na następną taką strukturę, jeżeli ten wskaźnik jest NULLem to znaczy, że z tego aktualnego wierzchołka nie ma już więcej możliwych przejść na inny wierzchołek.
